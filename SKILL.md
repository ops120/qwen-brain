...
| **创作能力（登录后）** | 对话流生图 / 生视频（3.0.2 起自动抓产物）；生图工作室 / PPT / 工作助理为入口型，未自动化 | 直接 `ask --prompt "画一张图：…"`，产物在 `files[]`（见「调用序列」） |
...
```bash
node "<skill-root>/scripts/qwb/cli.mjs" ask --prompt "画一张图：…" --thread new --json
```

- 命中生成意图时，文字回答先行返回，CLI 继续等图片/视频卡完成（`--artifact-wait`，默认 120 秒）
  并从卡片数据直链下载**原图**；产物在 `files[]`（含绝对路径），`mode` 变 `"artifact"`。
- **额度极低**：实测连续 ~6 次生图/小时即触发**静默限流**（挂死或秒回「我无法回答这个问题」，
  页面无任何提示，账号/IP 级）。每任务生图/生视频 ≤ 2 次；触限立即停手等待，**不要重试**。
...
5. `files[]` 非空时 `mode` 为 `"artifact"`：逐字引用 `text`，产物报绝对路径，并注明 `kind` 与尺寸。
...
- 生图/生视频另计：产物等待默认最多 +120 秒；**连续生成会触发静默限流**（无 UI 提示）。

## 能力边界

- **模型不可自选**：千问网页版的档位是「模式」而非模型（快速 / 思考研究）；
  底层模型（如 Qwen3.7-千问）由站点自动分配，CLI 不做模型切换。
- **生图 / 生视频**：对话流内 3.0.2 起已支持（`ask` 自动抓产物到 `files[]`）；
  生图工作室 / 生视频编辑器 / PPT / 工作助理是「入口型」工作流，仍未自动化，
  需要时告诉用户去网页手动操作。
- **生图额度硬上限低**：实测 ~6 次/小时触发**静默限流**——无任何页面提示，
  症状是请求挂死（`STREAM_STALLED`）或秒回罐头话术「我无法回答这个问题」；
  连「1+1」都被拒即触限，**停手等待**，勿重试（账号/IP 级，换浏览器指纹无效）。
